    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            <div class="mb-4 sm:mb-0">
                <h2 class="text-xl sm:text-2xl font-medium leading-6 text-white">Edit Campaign</h2>
        ...
        <div class="bg-gray-800 shadow-md rounded-lg overflow-hidden">
            <form action="{{ route('admin.causes.update', $cause->id) }}" method="POST" enctype="multipart/form-data" class="p-4 sm:p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $cause->title) }}" 
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" 
                                  class="w-full p-3 rounded-md bg-gray-700 text-white border border-gray-600 focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400">{{ old('description', $cause->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="goal" value="{{ old('goal', $cause->goal) }}" step="0.01" 
                               class="w-full p-3 rounded-md bg-gray-700 text-white border border-gray-600 focus:outline-none focus:border-orange-400 focus:ring-1 focus:ring-orange-400">
                        @error('goal')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        @if($cause->image)
                        <div class="mb-4">
                            <img src="{{ asset('storage/' . $cause->image) }}" alt="Campaign Image" class="h-32 sm:h-40 w-auto rounded-md object-cover">
                            <p class="text-gray-400 text-xs mt-2">Current image</p>
                        </div>
                        @endif
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <label class="flex items-start p-4 bg-gray-700 rounded-lg border border-gray-600 cursor-pointer hover:bg-gray-650 transition-colors">
                                <input type="radio" name="cause_type" value="general" {{ $cause->is_recent ? '' : 'checked' }} 
                            ...
                            <label class="flex items-start p-4 bg-gray-700 rounded-lg border border-gray-600 cursor-pointer hover:bg-gray-650 transition-colors">
                                <input type="radio" name="cause_type" value="recent" {{ $cause->is_recent ? 'checked' : '' }} 
                    ...
                    <div class="bg-gray-700 rounded-lg p-4 border border-gray-600" id="urgent_option" style="{{ $cause->is_recent ? '' : 'display: none;' }}">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_urgent" {{ $cause->is_urgent ? 'checked' : '' }} 
                    ...
                    <div class="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 pt-4">
                        <button type="submit" class="w-full sm:w-auto bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600 transition font-medium">
                            Update Campaign
                        </button>
